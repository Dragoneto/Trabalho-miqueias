...
async function exibirCaracterísticas(id) {
  try {
...
async function exibirNaves() {
  try {
    ...
    for (let i = 0; i < 3 && i < starships.results.length; i++) {
...
async function exibirPlanetas() {
  try {
    ...
    for (const planet of planets.results) {
      ...
      if (!isNaN(population) && population > 1000000000) {
        console.log(\n${planet.name});
...
async function main() {
  try {
    ...
    await exibirCaracteristicas(lastCharacterId);
    await exibirNaves();
    await exibirPlanetas();
    await exibirFilmes();
